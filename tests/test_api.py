...
class ApiTests(unittest.TestCase):
    def _utils_stub(self):
        return types.SimpleNamespace(
            ...
            list_cleanup_logs_with_sizes=lambda: (_ for _ in ()).throw(
                OSError("sensitive path info")
            ),
            read_cleanup_log=lambda *_a, **_k: (_ for _ in ()).throw(OSError("boom")),
            read_latest_cleanup_log=lambda *_a, **_k: (_ for _ in ()).throw(
                OSError("sensitive path info")
            ),
        )
    ...
    def _stats_stub(self):
        ...
        return types.SimpleNamespace(
            ...
            load_stats=lambda strict=False: (_ for _ in ()).throw(
            ...
            list_stats_backups=lambda: [
                {
                    "type": "stats",
                    "filename": "stats-1.json.bak",
                    "path": "/config/data/backups/stats-1.json.bak",
    ...
    def test_api_returns_generic_internal_error_message(self):
        ...
        self.assertEqual(
            logs_list_response.get_json()["error"], "Internal server error"
        )
    ...
    def test_api_exposes_stats_backups_and_status_config(self):
        ...
        notifications_stub = types.SimpleNamespace(
            get_recent_notification_fallbacks=lambda: [
                {"context": "daily cleanup report", "timestamp": "2026-04-15 05:45:00"}
            ]
        )
    ...
    def test_admin_stats_repair_and_repost_route_schedules_monthly_report(self):
        ...
        with isolated_module_import(
            ...
            admin_module.asyncio.run_coroutine_threadsafe = (

...
stats_group = app_commands.Group(
    name="stats", description="Cleanup statistics commands", parent=cleanup_group
)
...
async def stats_view(interaction: discord.Interaction):
    ...
    if top_channels:
        ...
        embed.add_field(
            ...
            value="\n".join(
                [ch_display(ch_id, ch_data) for ch_id, ch_data in top_channels]
            ),
            inline=False,
        )
...
@stats_group.command(
    name="channel", description="Show cleanup stats for a specific channel"
)
@app_commands.checks.has_permissions(administrator=True)
...
class StatsResetView(discord.ui.View):
    ...
    async def confirm(
        ...
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                "⛔ Only the person who triggered this reset can confirm it.",
                ephemeral=True,
            )
            return
        ...
        embed = discord.Embed(
            ...
            description=(
                f"✅ **{self.scope.capitalize()}** stats have been reset."
                if success
                else "⛔ Invalid scope — no stats were changed."
            ),
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.user.id:
            await interaction.response.send_message(
                "⛔ Only the person who triggered this reset can cancel it.",
                ephemeral=True,
            )
            return
        self.stop()

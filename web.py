...
if bool(WEB_AUTH_HEADER_NAME) != bool(WEB_AUTH_HEADER_VALUE):
    raise RuntimeError(
        "WEB_AUTH_HEADER_NAME and WEB_AUTH_HEADER_VALUE must either both be set or both be empty"
    )
...
def _split_stats_backups(
    backups: list[dict], limit: int = 10
) -> tuple[list[dict], list[dict]]:
    """Splits the shared backup inventory into stats and last-run lists."""
    stats_backups = [backup for backup in backups if backup.get("type") == "stats"][
        :limit
    ]
    last_run_backups = [
        backup for backup in backups if backup.get("type") == "last_run"
    ][:limit]
    return stats_backups, last_run_backups


def _build_history_channel_entry(
    ch_id, name: str, data: dict, channel_history: dict, all_time_channels: dict
) -> dict:
...
def _select_history_channel(
    ...
    if selected_id:
        for channel in history_channels:
            if (
                str(channel.get("id")) == selected_id
                or f"history-{channel.get('id')}" == selected_id
            ):
                return channel
...
def _check_admin_rate_limit() -> tuple[bool, int | None]:
    ...
    limit = (
        RUN_RATE_LIMIT_MAX_REQUESTS
        if request.path.startswith("/admin/run/")
        else ADMIN_RATE_LIMIT_MAX_REQUESTS
    )
    now = time.monotonic()
    ...
    with _rate_limit_lock:
        ...
        history[:] = [
            ts for ts in history if now - ts < ADMIN_RATE_LIMIT_WINDOW_SECONDS
        ]
        if len(history) >= limit:
            retry_after = max(
                1, int(ADMIN_RATE_LIMIT_WINDOW_SECONDS - (now - history[0]))
            )
...
def protect_web_ui():
    ...
    if WEB_AUTH_HEADER_NAME:
        ...
        if supplied != WEB_AUTH_HEADER_VALUE:
            log.warning(
                "Rejected web request with missing or invalid proxy auth header | path=%s",
                request.path,
            )
            abort(403)
    ...
    if request.method in {"POST", "PUT", "PATCH", "DELETE"}:
        ...
        if submitted != _csrf_token():
            log.warning(
                "Rejected web request with invalid CSRF token | path=%s", request.path
            )
            abort(403)
    ...
    if request.path.startswith("/admin/") and request.method in {
        ...
        if not allowed:
            log.warning(
                "Rejected admin request due to rate limiting | path=%s | requester=%s",
                request.path,
    ...
    if bot and bot.guilds:
        ...
        for ch_id, data in channel_map.items():
            ...
            configured_channels.append(
                {"id": ch_id, "name": name, "label": label, "category": category}
            )
...
def view_log(filename):
    ...
    try:
        ...
        context["available_logs"] = read_latest_cleanup_log(lines_requested=0)[
            "available_logs"
        ]
        context["log_entries"] = []
    except OSError as e:
...
def stats_page():
    ...
    for ch_id, ch_data in raw_channels.items():
        ...
        if category == "Standalone":
            ...
            cat_totals[category]["channels"].append(
                {"id": ch_id, "name": name, "count": count}
            )
        ...
        [
            {
                ...
                "count": data["count"],
                "channels": sorted(
                    data["channels"], key=lambda x: x["count"], reverse=True
                ),
            }
    ...
    category_summary = [
        (g["name"], {"count": g["count"], "channels": len(g["channels"])})
        ...
        for ch_id, data in channel_map.items():
            ...
            history_channels.append(
                _build_history_channel_entry(
                    ch_id, name, data, channel_history, all_time_channels
                )
            )
        history_channels.sort(
            key=lambda item: item["latest"]["timestamp"] if item["latest"] else "",
            reverse=True,
        )
    elif channel_history or all_time_channels:
        channel_ids = sorted(
            {str(ch_id) for ch_id in channel_history.keys()}
            | {str(ch_id) for ch_id in all_time_channels.keys()}
        )
    ...
    selected_drilldown_channel = _select_history_channel(
        history_channels, request.args.get("drilldown_channel")
    )
    ...
    context["history_channels_fallback"] = bool(history_channels) and not (
        bot and bot.guilds
    )
...
def audit_page():
    ...
    if bot and bot.guilds:
        ...
        notification_groups = {
            ch.get("notification_group")
            for ch in cfg.raw_channels
            if ch.get("notification_group")
        }
        report_groups = {
        ...
        summary["report_excluded"] = sum(
            1 for ch in cfg.raw_channels if ch.get("report_exclude", False)
        )
        summary["report_individual"] = sum(
            1 for ch in cfg.raw_channels if ch.get("report_individual", False)
        )
                ...
                audit_rows.append(
                    {
                        ...
                        "override": bool(
                            ch.get("days") and ch.get("days") != cfg.DEFAULT_RETENTION
                        ),
                        "deep_clean": bool(ch.get("deep_clean", False)),
                        "notification_group": ch.get("notification_group"),
                        "report_exclude": bool(ch.get("report_exclude", False)),
            ...
            audit_rows.append(
                {
                    ...
                    "kind": "Channel",
                    "category": mapped.get("category_name")
                    or (
                        discord_channel.category.name
                        if discord_channel and discord_channel.category
                        else "Standalone"
                    ...
                    "notification_group": mapped.get("notification_group")
                    or ch.get("notification_group"),
                    "report_exclude": mapped.get(
                        "report_exclude", ch.get("report_exclude", False)
                    ),
                    "report_individual": mapped.get(
                        "report_individual", ch.get("report_individual", False)
                    ),
                    "report_group": mapped.get("report_group")
                    or ch.get("report_group"),
                    "status": "configured",
                }
            )
...
def start_web_server():
    ...
    if WEB_HOST == "0.0.0.0" and not WEB_AUTH_HEADER_NAME:
        log.warning(
            "Web UI is listening on 0.0.0.0 without proxy-header auth configured. Prefer WEB_HOST=127.0.0.1 behind a reverse proxy."
        )
